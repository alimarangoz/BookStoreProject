...
<head>
    <meta charset="UTF-8">
    <title>Ales Book Store | Searching Result</title>
...
<div id="content">
    ...
    if(isset($_POST["search"]) && !empty($_POST["search"])){
        ...
        $searchq = preg_replace("[^0-9A-Za-z]","",$searchq);
        $query = "select * from books where name like '%$searchq%'";
        $statement = $connection->query($query);
        if($statement->rowCount() == 0 ){
            echo "<script type='text/javascript'>alert('There is no search result! You are redirecting to home page.')</script>";
            ...
            foreach ($search as $row){
                ...
                $stmt2->execute();
                $authorSearch = $stmt2->fetchAll(PDO::FETCH_ASSOC);
                ...
                echo "<div class='item'>
                      ...
                      <div class='bookNames' style='color: #007bff'>".$authorSearch[0]['name']."</div>
